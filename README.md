...
- A deferred value cannot break out of its own patch: a `</template>` that would close it (one that does not close a `<template>` the value opened itself) is escaped and rendered as text, and so is `<plaintext>`, which nothing could close. Balanced nested `<template>` elements, and `</template>` inside an attribute value, a comment or a `<script>`, pass through unchanged. That holds whether a browser reads `<noscript>` (or a raw text element inside `<select>`, `<svg>` or `<math>`) as raw text or as markup: inside one, markup that could end it early under the other reading is escaped, and so are `<template>` tags. Malformed markup there can make the rest of the patch part of that element (`<noscript>a<b</noscript><p>Visible</p>` hides the paragraph where scripting is enabled). Inside `<svg>` / `<math>`, `<template>` tags are escaped too (they are SVG / MathML elements there, which an end tag can close implicitly), and so is a `<![CDATA[` section with a `>` before its `]]>`. For an `<svg>` / `<math>` that is self-closing (`<svg/>`) or has an element nested in a `<foreignObject>`, `<desc>`, `<title>`, `<mi>`, `<mo>`, `<mn>`, `<ms>`, `<mtext>` or `<annotation-xml>`, rendu cannot tell where it ends, so that treatment lasts until the end of the patch (or of the nested `<template>` holding it).
