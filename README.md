...
The cookie header is only parsed on first access. All traps are backed by the parsed map so `get`, `in`, `Object.keys()`, spread and `JSON.stringify()` are consistent. It converts to a string like a plain object and `util.inspect()` / `console.log()` show the cookies.
...
- rendu keeps a deferred value inside its own patch: a `</template>` that would close it (one that does not close a `<template>` the value opened itself) is escaped and rendered as text, and so is `<plaintext>`, which nothing could close. Balanced nested `<template>` elements, and `</template>` inside an attribute value, a comment or a `<script>`, pass through unchanged. This guard follows the HTML tokenizer, not tree construction, so truncated or malformed markup inside `<noscript>`, `<select>` or `<svg>` / `<math>`, or content that starts with `<col>`, can end a patch early or leave it open (hiding later panels). It is not a sanitizer.
...
`htmlspecialchars(value)`: `null` and `undefined` become `""` and a promise resolves to its escaped value. Any other value is escaped as `String(value)` (write `defer()` with `<?= ?>`, as above).
